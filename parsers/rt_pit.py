...
def main():
  ...
  rt_pit = [0] * (int((requests[-1][0].ts() - requests[0][0].ts()) / 50000) + 1)
  for request in requests:
    if request[-1].ts() - request[0].ts() > 3000000:
      continue
    slot = int((request[0].ts() - requests[0][0].ts()) / 50000)
    rt_pit[slot] = max(rt_pit[slot],
        int((request[-1].ts() - request[0].ts()) / 1000))
  with open("rt_pit.data", 'w') as rt_pit_file:
    for (slot_no, rt) in enumerate(rt_pit):
      rt_pit_file.write("%s %s\n" % (slot_no * 50, rt))
  # Print statistics.
  print("Min PIT RT: %s ms" % (numpy.min(rt_pit)))
  print("Average PIT RT: %s ms" % (numpy.average(rt_pit)))
  print("Median PIT RT: %s ms" % (numpy.median(rt_pit)))
  print("Max PIT RT: %s ms" % (numpy.max(rt_pit)))
